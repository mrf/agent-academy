...
export const MISSIONS: Mission[] = [
  ...
  // ─── MISSION 06 ─────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        type: "print",
        text: "Here's where operatives get careless. Every interaction adds to the context window. Your messages, Claude's responses, file contents from Read operations, command outputs, even system instructions — it all consumes space.\n\nWhen the context window fills up, your agent automatically summarizes older messages to make room. Key information is preserved, but the full detail of earlier exchanges is lost.",
      },
      {
        type: "print",
        text: "This is why context management matters. Without it, long sessions silently lose critical information. Your agent might forget instructions you gave earlier, re-read files it already processed, or lose track of the task.\n\nThe good news: your agent gives you tools to manage this. That's what this mission is about.",
      },
      {
        type: "print",
  ...
  // ─── MISSION 09 ─────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        type: "print",
        text: "Git integration is part of the operative's core training. Your agent can stage files, create commits with meaningful messages, check diffs, view logs, and manage branches — all through the Bash tool.\n\nYour agent can push to remotes too, but many teams prefer to keep that as a human decision. Pushing affects shared state — other developers, CI pipelines, deployments. A best practice: let your agent handle local git ops freely, but review before pushing.",
      },
      {
        type: "quiz",
        question:
          "What's the recommended approach for your agent and git pushes?",
        options: [
          "Your agent should handle everything including force pushes",
          "Your agent can push, but it's best practice to review before pushing to shared remotes",
          "Your agent only reads git status, never writes",
          "Your agent should never touch git at all",
        ],
        correct: 1,
        explanation:
          "Your agent can do any git operation, but pushes affect shared state — other developers, CI, deployments. Best practice: let your agent handle local operations freely (staging, committing, branching, diffing) and review before pushing to remotes.",
      },
      {
        type: "quiz",
        question:
